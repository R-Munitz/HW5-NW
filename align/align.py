...
class NeedlemanWunsch:
    ...
    def align(self, seqA: str, seqB: str) -> Tuple[float, str, str]:
        ...
        self._gapA_matrix = np.full((row, col), -np.inf)
        self._gapB_matrix = np.full((row, col), -np.inf)

        # initialize backtrace matrices with zeros - 2.0 move to backtrace method?
        ...
        for i in range(1, row):     # move down rows in first col
        ...
        """
        notes: NW algorithm - double check up/left are correct
        ...
        up move - from (i-1, j), align seqA[i-1] with a gap in seqA, add gap to seqA. score = previous score + gap penalty
        left move - from (i, j-1),align seqB[j-1] with a gap in seqB, add gap to seqB. score = previous score + gap penalty
        ...
        for i in range(1, row):  # skip first row - gap penalties already filled in
            for j in range(1, col):  # skip first col - gap penalties already filled in
                ...
                diagonal_score = self._align_matrix[i - 1][j - 1] + self.sub_dict[(seqA[i - 1], seqB[j - 1])] # look up score in substitution matrix


                # up move - adding gap to seqA
    ...
    def _backtrace(self) -> Tuple[float, str, str]:
        ...
        while i > 0 or j > 0:
            ...
            if move == 0:  # diagonal move
                seqA_align_list.append(self._seqA[i - 1])  # add last res of seqA to alignment
                seqB_align_list.append(self._seqB[j - 1])  # add last res of seqB to alignment

                # move diagonally
                i -= 1
                j -= 1

            elif move == 1:  # Up move - add gap to seqA - 2.0 reversed
                seqA_align_list.append(self._seqA[i - 1])
                seqB_align_list.append("-")
                i -= 1

            else:  # Left move - add gap to seqB - 2.0 reversed
                seqA_align_list.append("-")
                seqB_align_list.append(self._seqB[j - 1])
